...
export default class tablaSimbolos{
    ...
    public getAnterior(){return this.tablaAnterior;}
    public setAnterior(anterior:tablaSimbolos){this.tablaAnterior=anterior;}
    public getTabla(){return this.tablaActual;}
    public setTabla(Tabla:Map<String, Simbolo>){this.tablaActual=Tabla;}
    public setVariable(simbolo:Simbolo){
        for(let e: tablaSimbolos=this; e!=null;e=e.getAnterior()){
            let encontrado:Simbolo=<Simbolo>(e.getTabla().get(simbolo.getidentificador()));
            if(encontrado!=null){
